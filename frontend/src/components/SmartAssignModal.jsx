...
export default function SmartAssignModal({
  ...
  useEffect(() => {
    ...
    rankTechniciansForOperation({
      ...
      .catch(e => setError(e.message || t('modals.smartAssign.errorRanking')))
      ...
      <div className="relative z-10 bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <div className="flex items-center gap-3 px-5 py-3 border-b border-border bg-gradient-to-r from-purple-50 to-purple-100">
          ...
          <div className="flex-1">
            <h3 className="text-base font-bold text-gray-900">{t('modals.smartAssign.title')}</h3>
            <p className="text-xs text-gray-600">
              {t('modals.smartAssign.subtitle', { specialty: specialty || '—', shift, hours: plannedHours })}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading && <p className="text-center text-sm text-muted-foreground py-8">{t('modals.smartAssign.calculating')}</p>}
          {error && <p className="text-center text-sm text-red-600 py-8">{error}</p>}
          {!loading && !error && candidates.length === 0 && (
            <p className="text-center text-sm text-muted-foreground py-8">{t('modals.smartAssign.noCandidates')}</p>
          )}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-bold text-purple-900 mb-1">
                    {t('modals.smartAssign.recommendation')}{' '}
            ...
            <table className="w-full text-xs">
              <thead className="bg-muted">
                <tr className="border-b border-border">
                  <th className="px-2 py-1.5 text-left font-semibold">{t('modals.smartAssign.colTechnician')}</th>
                  <th className="px-2 py-1.5 text-left font-semibold">{t('modals.smartAssign.colSpec')}</th>
                  <th className="px-2 py-1.5 text-left font-semibold">{t('modals.smartAssign.colSkills')}</th>
                  <th className="px-2 py-1.5 text-left font-semibold">{t('modals.smartAssign.colShift')}</th>
                  <th className="px-2 py-1.5 text-right font-semibold">{t('modals.smartAssign.colHh')}</th>
                  <th className="px-2 py-1.5 text-right font-semibold">{t('modals.smartAssign.colScore')}</th>
                    ...
                    <tr key={c.worker_id} className={`border-b border-border ${isAiPick ? 'bg-purple-50 hover:bg-purple-100' : 'hover:bg-muted/30'}`}>
                      ...
                      <td className={`px-2 py-1.5 text-right font-bold ${scoreColor}`}>
                        ...
                        <span className="text-[9px] block font-normal text-muted-foreground" title={JSON.stringify(c.breakdown)}>
                          {t('modals.smartAssign.criteriaCount', { count: Object.keys(c.breakdown || {}).length })}
                        </span>
                      </td>
                      <td className="px-2 py-1.5">
                        <button
                          ...
                          <Check className="w-3 h-3" /> {t('modals.smartAssign.assign')}
                        </button>
                      </td>
                    </tr>
                  );
        ...
        <div className="px-5 py-2 border-t border-border bg-muted/30 text-[10px] text-muted-foreground">
          {t('modals.smartAssign.scoreLegend')}
        </div>
      </div>
    </div>
  );
