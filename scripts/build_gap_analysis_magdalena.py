...
KEY_MAP: list[tuple[str, str, str, str, str]] = [
    ...
    # KPIS ─────────────────────────────
    (r"KPI.*producci|toneladas.*proces|t/d|t/h|utilizaci.n", "PARCIAL", "Production tab Executive · usa proxies, falta data plant SCADA real", "Q3-2026", "Cliente"),
    ...
    (r"opex.*real.*plan|capex.*real.*plan|gasto.*plan", "PARCIAL", "Costos tab por OT · agregado por CeCo pendiente", "Tanda 0E", "David"),
    ...
    (r"plan.*matriz|plan.*estrat|strategy plan", "PARCIAL", "Reliability strategy + RCM + maintenance-strategy-drivers context", "Q3-2026", "David"),
    ...
    (r"^¿.*completar.*programa\?", "PARCIAL", "Schedule compliance KPI + reprogram endpoint · gate explícito pendiente", "SP8", "David"),
    (r"^¿.*tiempo.*adicional|^¿.*recursos.*adicional", "PARCIAL", "Reschedule + extender HH endpoints · workflow approval extra pendiente", "SP8", "David"),
    (r"^¿.*disponibl|^¿.*alcanzar|^¿.*lograr", "PARCIAL", "Health scoring + capacity checks · decision tree explícito pendiente", "SP8", "David"),
    ...
    (r"safety.*KPI|HSE.*indicator|accidentes|seguridad", "PARCIAL", "HSE tab Executive · falta integración incidentes plant", "Q3-2026", "Cliente"),
