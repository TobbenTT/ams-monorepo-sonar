"""Replace AI analyze endpoint with smart fuzzy matching + synonyms."""
import re

with open('/root/ASSET-MANAGEMENT-SOFTWARE/api/routers/data_import.py', 'r') as f:
    code = f.read()

start_marker = '# -- AI Auto-Configure --'
...
if idx == -1:
    ...
    idx = code.find(start_marker)
if idx == -1:
    print("Marker not found, looking for @router.post ai-analyze")
    idx = code.find('@router.post("/ai-analyze")')
    if idx > 0:
        # go back to find the imports
        idx2 = code.rfind('\nclass AIAnalyze', 0, idx)
        if idx2 > 0:
            idx = idx2
        else:
            idx = code.rfind('\nfrom pydantic', 0, idx)
...
if idx == -1:
    print("ERROR: Could not find AI section")
    exit(1)

prefix = code[:idx]

new_ai = r'''# -- AI Auto-Configure --
...
with open('/root/ASSET-MANAGEMENT-SOFTWARE/api/routers/data_import.py', 'w') as f:
    f.write(prefix + new_ai)

print("Done - smart AI with fuzzy matching, synonyms, sample data context")
